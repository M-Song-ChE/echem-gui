...
class FileManagerMixin:
    ...
    Expects the host class to have:
        ...
        self.file_listbox, self.cycle_listbox
    ...
    def _load_files(self):
        ...
        for path in paths:
            ...
            if "cycle number" in df_raw.columns:
                selected = sorted(int(c) for c in df_raw["cycle number"].unique())
        ...
        # Auto-select the last loaded file but do NOT replot
        # (the user's current plot is preserved until they click Plot)
    ...
    def _switch_active_file(self, short):
        ...
        # Suppress auto-replot while rebuilding the cycle listbox
        ...
        if "cycle number" in df.columns:
            ...
            for c in cycles:
                self.cycle_listbox.insert(tk.END, str(c))
            saved = entry["selected_cycles"]
            for i in range(self.cycle_listbox.size()):
                if int(self.cycle_listbox.get(i)) in saved:
                    self.cycle_listbox.select_set(i)
