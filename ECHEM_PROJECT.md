...
- **Per-catalyst corrections** (`catalyst_corrections`): `sentry["catalyst_corrections"][catalyst_id]` = `{"r_sol_n2": float, "r_sol_o2": float, "e_ref": float, "area": str, "ecsa": str}` — each catalyst within a sample group has fully independent IR/RHE correction values. `area` is electrode area (cm²) for J normalization; `ecsa` is ECSA_Hupd (cm²) used for specific activity in the Extract Report window and auto-populated into SA/Comparison window ECSA inputs. Clicking a GC curve shows GC's values; clicking Pt shows Pt's. This is the correct model for measurements on different cells that happen to be in the same sample group.
...
- **StringVar traces for correction sync**: traces on `r_sol_n2_var`, `r_sol_o2_var`, `e_ref_var`, `area_var`, `ecsa_var` call `_on_corr_var_trace(key, var)` on every keystroke; writes immediately to `catalyst_corrections[active_catalyst][key]` guarded by `_switching_sample`; `area` and `ecsa` keys are stored as raw strings (not float-cast); this eliminates FocusOut race conditions where switching samples/catalysts could write old UI values into the new selection
...
- **Kinetic current (Jᵏ) — single source of truth**: three module-level helpers sit next to `_find_half_wave` and are the ONLY way Jᵏ is produced anywhere in the panel:
  - **`_kl_fit_at_E(curves, e_val)`** — `curves` is an iterable of `(E_arr, J_arr, rpm)`. Interpolates each curve at `e_val`, keeps only finite **cathodic** (`j < 0`) points at `rpm > 0`, then least-squares fits `y = 1/|J|` against `x = ω^-½` (`ω = 2π·rpm/60`). Returns `dict(j_k_abs, j_k, j_k_se, slope, intercept, r2, npts, rpms, rpms_sorted, x, y, note)`; `j_k_abs = 1/intercept`, `j_k = -j_k_abs` (signed cathodic). Returns **`None`** when fewer than 2 *distinct* RPMs survive filtering. Sets `j_k_abs = nan` with an explanatory `note` when `intercept <= _KL_MIN_INTERCEPT` (1e-6) — an unphysical fit, not a crash. `j_k_se` is the intercept's standard error propagated through `1/x` (`nan` when `npts < 3`, since a 2-point line has no residual df).
  - **`rpms` is aligned element-wise with `x`/`y`** — `x = ω^-½` *decreases* as RPM rises, so never sort one without the others. Use `rpms_sorted` for display only. (Annotating KL scatter points with `np.sort(x)` against unsorted `rpms` mislabels every point.)
  - **`_kl_n_electrons(slope, b_factor)`** — `n = 1/(|slope|·B_factor)`, where `B_factor = 0.62·F·D^(2/3)·ν^(-1/6)·C·1000` (the ×1000 converts A→mA).
  - **`_kl_kinetic_curve(curves, e_lo, e_hi, n_grid=60)`** — runs one `_kl_fit_at_E` per point of a grid spanning the **overlap** of every curve's E window, giving mass-transport-free `Jᵏ(E)` for Tafel analysis. Returns `(E_used, Jk_abs, n_rpm)`, or `(None, None, n_rpm)` when `n_rpm < 2` or fewer than 3 grid points yield a finite fit.
  - **Callers**: `_open_sa_window` (one fit per `(sample, catalyst)` group per E), `_open_report_window` (one fit per catalyst over all its RPMs), `_open_comparison_window`'s Kinetic tab (`_cat_kl_curves(cat)` → SA bars + `_kl_kinetic_curve` → Tafel), `_open_tafel_window` (KL branch), and `_open_kl_window` itself.
  - **Removed 2026-08 — do not reintroduce**: the single-curve estimate `Jᵏ = J·J_lim/(J_lim − J)` with `J_lim = min(J)`. It is not the accepted method and inherits every plateau defect of the one curve it reads (verified: one 8 % plateau noise spike → −90 % SA bias at 0.80 V; a 15 % tilted plateau → −92 %). `min(J)` legitimately remains in use for `_find_half_wave` (E½ *is* defined off the measured curve's plateau) and for the Levich / LC-Compare windows (which are *about* J_lim), but never for Jᵏ.
...
- **Scroll zoom**: `_on_scroll` — `"up"` → `factor = 1/1.15` (zoom in), `"down"` → `factor = 1.15` (zoom out); applies symmetrically around cursor `(xdata, ydata)` on both axes
- **Catalyst click-select + highlight**: `_try_annotate` detects nearest curve, extracts `_clicked_cat` via `re.match(r'^\[(\w+)\]', label)`, stores to `sentry["highlighted_cat"]`, calls `_apply_orr_highlight(sample_name)`; `_apply_orr_highlight` iterates `ax.get_lines()`, skips `label.startswith("_")`, sets matching-catalyst lines to `alpha=1.0, zorder=5`, others to `alpha=0.15, zorder=2`; `hl_cat=None` resets all to `alpha=1.0, zorder=2`; right-click sets `sentry["highlighted_cat"]=None` then `_apply_orr_highlight` then `_clear_ann`; `_plot_sample` calls `_apply_orr_highlight` after reflines so highlight survives every replot; `highlighted_cat` is NOT in `_SAMPLE_RUNTIME` so it persists across replots
...
- **Curve selectors in the analysis windows**: one `dict[idx → BooleanVar]` per window, all filtered inside that window's own local `_compute()` (there is no `_kl_compute`; every window names its callback `_compute`). Names differ per window and are NOT interchangeable: `_tsel_vars` (Tafel), `_ksel_vars` (KL), `_ssel_vars` (SA), `_lsel_vars` (LC Compare). Since the KL rewrite these checkboxes changed meaning in the Tafel/SA/Comparison windows: unchecking an RPM removes it from the **fit**, not merely from a drawn line, so it moves the reported Jᵏ/SA/slope. They are seeded by `_iter_analysis_records()` → `_build_curve_records(sn, include_disabled=True)`, so a pair whose **plot** checkbox is off still gets a row (label suffixed `·hidden`), ticked on, and counts in the fit — plot visibility and fit membership are independent. `_get_curves_for_sample()` / the Excel export keep the default `include_disabled=False` and still honour the plot checkbox. Each window builds a `_sel_on` list alongside its curve list for the `·hidden` labels; `_gradient_recolor` preserves order so the two stay index-aligned. **SA's per-curve checkboxes had no `trace_add("write", _schedule)`** — toggling one changed nothing until "Compute SA" was pressed; now wired like the other windows
...
- **Internal transport correction (Shih 2008)** — `_film_limited_fit(curves)` fits `1/|J_L,obs| = 1/j_f + (1/B)·ω^-½` over each curve's most cathodic point, returning `j_f`, `j_f_se` (1σ via the intercept SE, nan at n<3), `slope`, `r2`. `_eq6_true_kinetic(jk_app, j_f)` applies the paper's Eq. 6 and returns `(value, flag)` with flags `ok` / `near_limit` (>0.8·j_f, `_ITC_WARN_RATIO`) / `exceeds` (≥ j_f, no value) / `no_film`. **Both arguments must share a normalisation**; the code works in mA/cm²_geo throughout and converts to SA afterwards, which is algebraically identical to the paper's specific-current form since the geo→specific factor is constant per sample. `_kl_fit_at_E`'s intercept is `j_k,app` (Eq. 5), **not** the intrinsic current — the pre-existing Jk/SA columns are apparent values. The correction is negligible near 0.9 V (`j_k,app ≪ j_f`) and only bites at higher overpotential. Consumed by `_open_itc_window` (4-panel diagnostic + per-catalyst MODEL CONSISTENT / PARTIALLY CONSISTENT / NOT SUPPORTED verdict) and by the report window's `j_f` / `Jk_true` / `SA_true` / `film fit R2` / `ITC flag` columns. Deliberately NOT implemented: estimating `j_f` from catalyst loading (the paper's Fig. S1 `j_f ∝ 1/loading` gives R²=0.10 on this lab's data — here `j_f` tracks CA chemistry, not layer thickness) and forcing a −120 mV/dec Tafel slope
- **Analysis windows** — 8 popup `tk.Toplevel(self)` windows: `_open_tafel_window`, `_open_kl_window`, `_open_sa_window`, `_open_levich_window`, `_open_lc_comparison_window`, `_open_comparison_window` (last has 4 sub-tabs in a Notebook: Ratio / Levich / KL / Kinetic), `_open_report_window`; `_open_sa_window` and the Kinetic tab of `_open_comparison_window` pre-populate their ECSA-per-catalyst entry fields from stored `catalyst_corrections[cat]["ecsa"]` values (first non-empty value per catalyst across all samples wins)
  - **`_open_report_window`**: for all visible plotted samples, emits **per-RPM** columns `I at E (mA)`, `JL (mA/cm²)` and `%Theo (%)` at RPMs [400, 900, 1600, 2500] (`%Theo` = |JL| / Levich prediction × 100, using the window's own Electrolyte/n combobox — default `0.1 M HClO₄`, n=4 — and blank without an electrode area, since the theory is a current density) (closest RPM within ±50 rpm of each target), plus a **per-catalyst** block `Jk at E` / `Jk at E error` / `SA at E` / `SA at E error` / `KL R2` / `n_RPM` (errors are `fit["j_k_se"]`, and that ÷ ECSA; blank when `npts < 3`). Headers are 3-line strings and the TSV builder quotes any cell containing a newline/tab/quote, so Excel gets one wrapped header cell per column; the on-screen table flattens them via `" ".join(h.split())`. **The TSV goes to the clipboard through `copy_text_to_clipboard()` (plotting.py), not `clipboard_append`** — Tk rewrites every LF as CRLF on Windows, including inside a quoted cell, so the row breaks and the header's own line breaks became indistinguishable and Excel scattered the table across ~30 rows. The helper writes CF_UNICODETEXT directly (CRLF between rows, LF inside quoted cells — Excel's own format) and falls back to the Tk clipboard off Windows. "Copy TSV" also runs `_compute_and_fill()` first when nothing has been computed yet, instead of silently doing nothing. Curves come from `_get_curves_for_sample(sn, include_disabled=True)` — the report has no per-run selector, so plot visibility must not gate it from ONE `_kl_fit_at_E` over every RPM that catalyst has. SA is per-catalyst, **not** per-RPM — a KL extrapolation is defined over the RPM series, not within one member of it. Target E set by user; uses ECSA_Hupd from `catalyst_corrections[cat]["ecsa"]`; "Copy TSV (→ Excel)" copies a clipboard-ready tab-separated table; cells blank/N/A when data or ECSA is missing, and `"N/A (< 2 RPM)"` when the catalyst has too few rotation rates
  - **`_open_tafel_window`**: the "Mass-transport-correct J → Jᵏ (KL, multi-RPM)" checkbox switches the whole plot between two shapes — checked (**the default**) groups selected curves by `(sample, catalyst)` and draws **one line per group** from `_kl_kinetic_curve`; unchecked draws **one line per curve** (raw J, RPM-dependent slope). The default was flipped to on because a Tafel slope read off raw J splits one catalyst into one line per RPM, each with a different slope — visually and physically wrong, and the first thing the window used to show. When the KL branch plots nothing, an on-axes message explains why rather than leaving blank axes. The shared inner `_fit_and_plot(x_log_j, y_E, color, label)` does the `E = a + b·log₁₀|J|` fit for both branches. Sanity check on synthetic KL data: uncorrected gives −278/−214/−181/−161 mV/dec across four RPMs, KL-corrected gives −60.0 mV/dec for all of them
...
- **Module-level helpers**:
  ...
  - `_is_hupd_cv_file(name)` — `.mpr`/`.txt` + standalone `CVh` token (`_CVH_PAT = r'(?<![a-z0-9])cvh(?![a-z0-9])'`) + the file's OWN trailing technique segment being CV (via ORR's `_tech_tail`). The trailing-tech check is what drops the `_NN_CA` pre-step that every CVh run writes alongside the `_NN_CV` sweep; reading the *last* `_NN_TECH` segment (not the first match anywhere) matters because BioLogic re-runs embed a prior step's context earlier in the filename
  - `_folder_corrections(folder)` — wraps ORR's `_extract_folder_corrections` and maps it to Hupd's two fields: OCV → `e_ref`, EIS1 (`r_sol_n2`) → `r_sol`. Hupd has no N2/O2 split so EIS2 is unused
...
- **File loading**: `_add_paths(paths, corr=None)` → `(added, errors)` does the per-path work (dedupe by `_n` suffix, read, cycle detection, entry creation) and optionally pre-fills each new entry's `r_sol`/`e_ref` from *corr*; `_finish_load()` selects the first file when nothing is active. Shared by `_load` (file dialog) and `_load_folder`
- **`_load_folder`** (mirrors the ORR panel's loader): picked folder with top-level CVh files → treated as a single sample and loaded directly (never descends, so nested `reverse` folders are skipped); otherwise treated as a parent → immediate sub-folders containing CVh files become candidates in `_choose_folders_dialog(candidates)` (checkbox modal, all checked, Select/Deselect-All). `_load_one_folder(folder)` → `(added, errors, corr)` applies **that folder's own** `_folder_corrections` to its files, so each sample keeps independent R_sol/E_ref. Verified on `Reference dataset/2. VA`: 24 sample folders → exactly 1 CVh file each, per-folder corrections applied
- **`self._status`** — grey wraplength label under the load buttons, written by `_log(msg)`; reports load counts + the detected E_ref/R_sol per folder (HupdPanel has no results log pane like ORR's)
- **Left panel**: Load Files / Load Folder / Remove buttons + status label + `tk.Listbox`; Cycle combobox (`_cycle_cb` / `_v_cycle`); Correction LabelFrame (R_sol / E_ref per file); Parameters LabelFrame (scan_rate, dl_lo/hi, e1/e2, q_ref, geo_area); **Compute All** button; results `ttk.Treeview` (columns: File, Q_H [µC], ECSA [cm²], RF)
...
- **Default parameters** (`_DEF`): `scan_rate="20"`, `dl_lo="0.40"`, `dl_hi="0.50"`, `e1="0.025"`, `e2="0.40"`, `q_ref="210"`, `geo_area="0.1963"`
...
- **ORR StringVar trace write-through** — the correction UI vars (`r_sol_n2_var`, `r_sol_o2_var`, `e_ref_var`, `area_var`, `ecsa_var`) each have a `trace_add("write", ...)` that immediately writes the new value to `catalyst_corrections[active_catalyst][key]`; `area` and `ecsa` keys are stored as raw strings (not float-cast); this is the correct way to avoid FocusOut race conditions where switching away from a UI widget can fire FocusOut after `active_sample` / `_active_catalyst` has already changed
