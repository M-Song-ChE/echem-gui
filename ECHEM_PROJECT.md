...
- **N2/O2 pairing**: each call to `_add_files_to_sample` processes files individually; for each file it first searches existing pairs for a matching `(catalyst_id, rpm_id)` slot where that gas position is empty, and fills it (merge); only creates a new pair when no mergeable slot exists; auto-suffixes the catalyst label (`cat_2`, `cat_3`, …) when a `(label, rpm_id)` collision is unavoidable; this means N2 and O2 files for the same pair can be added in separate "Add" clicks without creating duplicate incomplete pairs
- **Pair table**: dynamic scrollable frame rebuilt by `_rebuild_pair_table`; each row contains: `tk.Checkbutton` (bound to `pair["enabled"]`, triggers `_auto_replot` on toggle) + Catalyst `tk.Entry` + RPM `tk.Entry` + N2 label (fills remaining left half, `pack(side=LEFT, fill=X, expand=True)`) + O2 label (fills remaining right half, `pack(side=RIGHT, fill=X, expand=True)`) + ✕ remove button (`side=RIGHT`); N2/O2 labels have no fixed width and expand to fill available space; `_save_pair` on FocusOut/Return; **catalyst rename cascade** — when a catalyst name changes, `_save_pair` renames all pairs sharing the old `catalyst_id`, renames keys in `catalyst_corrections` and `catalyst_styles`, updates `_active_catalyst`, then calls `_rebuild_pair_table` + `_update_catalyst_selector`
