...
- **Analysis windows** — 7 popup `tk.Toplevel(self)` windows: `_open_tafel_window`, `_open_kl_window`, `_open_sa_window`, `_open_levich_window`, `_open_lc_comparison_window`, `_open_comparison_window` (last has 4 sub-tabs in a Notebook: Ratio / Levich / KL / Kinetic), `_open_report_window`; `_open_sa_window` and the Kinetic tab of `_open_comparison_window` pre-populate their ECSA-per-catalyst entry fields from stored `catalyst_corrections[cat]["ecsa"]` values (first non-empty value per catalyst across all samples wins)
  ...
  - **`_open_tafel_window`**: the "Mass-transport-correct J → Jᵏ (KL, multi-RPM)" checkbox switches the whole plot between two shapes — unchecked draws **one line per curve** (raw J, RPM-dependent slope); checked groups selected curves by `(sample, catalyst)` and draws **one line per group** from `_kl_kinetic_curve`. The shared inner `_fit_and_plot(x_log_j, y_E, color, label)` does the `E = a + b·log₁₀|J|` fit for both branches. Sanity check on synthetic KL data: uncorrected gives −278/−214/−181/−161 mV/dec across four RPMs, KL-corrected gives −60.0 mV/dec for all of them
