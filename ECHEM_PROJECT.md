...
- **Analysis windows** — 7 popup `tk.Toplevel(self)` windows: `_open_tafel_window`, `_open_kl_window`, `_open_sa_window`, `_open_levich_window`, `_open_lc_comparison_window`, `_open_comparison_window` (last has 4 sub-tabs in a Notebook: Ratio / Levich / KL / Kinetic), `_open_report_window`; `_open_sa_window` and the Kinetic tab of `_open_comparison_window` pre-populate their ECSA-per-catalyst entry fields from stored `catalyst_corrections[cat]["ecsa"]` values (first non-empty value per catalyst across all samples wins)
  - **`_open_report_window`**: for all visible plotted samples, emits **per-RPM** columns `I at E (mA)` and `JL (mA/cm²)` at RPMs [400, 900, 1600, 2500] (closest RPM within ±50 rpm of each target), plus a **per-catalyst** block `Jk at E (mA/cm², KL)` / `SA at E (mA/cm²_ECSA, KL)` / `KL R2` / `n_RPM` from ONE `_kl_fit_at_E` over every RPM that catalyst has. SA is per-catalyst, **not** per-RPM — a KL extrapolation is defined over the RPM series, not within one member of it. Target E set by user; uses ECSA_Hupd from `catalyst_corrections[cat]["ecsa"]`; "Copy TSV (→ Excel)" copies a clipboard-ready tab-separated table; cells blank/N/A when data or ECSA is missing, and `"N/A (< 2 RPM)"` when the catalyst has too few rotation rates
