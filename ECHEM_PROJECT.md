...
- `self.files = OrderedDict[str, dict]` keyed by short filename
  ...
  - **Per-file gradient fields** (defaulted in `_switch_active_file`): `"cycle_gradient"` (bool, default `True`), `"cycle_reverse"` (bool, default `True`), `"lightness_step"` (str float, default `"0.15"`); saved by `_save_active_state` / `_on_gradient_change`
...
- **Cycle gradient defaults (all tabs)** — `cycle_reverse=True`, `lightness_step="0.15"` — set in `file_manager.py` on load and as fallback defaults throughout all panels
